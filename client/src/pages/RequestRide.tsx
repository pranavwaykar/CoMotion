...
export default function RequestRide() {
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const res = await api.post('/api/rides/requests', {
        period,
      ...
      setMessage(`Request created: ${res.data.request._id}`);
    } catch (err: any) {
  ...
  return (
    <Stack spacing={2} maxWidth={560} margin="0 auto">
      <Typography variant="h6">Request a Ride</Typography>
      ...
      <Button variant="contained" onClick={submit}>Create Request</Button>
    </Stack>
  );
}
